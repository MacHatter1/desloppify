...
def test_update_living_plan_after_resolve_no_living_plan(monkeypatch) -> None:
    monkeypatch.setattr(living_plan_mod, "has_living_plan", lambda: False)
...
def test_update_living_plan_after_resolve_fixed_flow(monkeypatch, capsys) -> None:
    ...
    calls: list[str] = []
    monkeypatch.setattr(living_plan_mod, "has_living_plan", lambda: True)
    monkeypatch.setattr(living_plan_mod, "load_plan", lambda: plan)
    ...
    monkeypatch.setattr(living_plan_mod, "save_plan", lambda _plan: calls.append("save"))
